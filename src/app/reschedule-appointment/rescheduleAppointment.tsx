...
export default function Page() {
  ...
  const fetchAppointment = async () => {
    try {
      const res = await axios.get(`/api/appointment/${appointmentId}/reshedule-appointment`, {
        headers: {
          'Content-Type': 'application/json',
        }
      })
  ...
  const generateSlots = (barberName: string) => {
    ...
    if (!busySlotTimeLine[barberName]) {
      const slot = generateAvailableSlots([], salon?.openingTime!, salon?.closingTime!, 15, appointment.duration)
      setAvailableSlot(slot);
    } else {
      const slot = generateAvailableSlots(busySlotTimeLine[barberName], salon?.openingTime!, salon?.closingTime!, 15, appointment.duration)
      setAvailableSlot(slot);
    }
  }
  ...
  const handleResheduleAppointment = async () => {
    const data = {
    ...
    try {
      ...
      const res = await axios.patch(`/api/appointment/${appointmentId}/reshedule-appointment`, data);
      toast.success(res.data.message || "Appointment Reshedule successfully.")
      router.push('/my-appointments')
    } catch (error) {
      const err = error as AxiosError<{ error: string }>;
      toast.error(err.response?.data.error || "somethin went worng.")
        ...
        <div className="container mx-auto max-w-4xl">
          ...
          >
            <h1 className="text-3xl md:text-4xl font-display font-bold text-foreground mb-2">
              <span className="text-green-500">Reschedule</span> Appointment
            </h1>
            <p className="text-muted-foreground">
              Complete the steps below to reschedule your visit
            </p>
          </motion.div>
              ...
              <AnimatePresence mode="wait">
                ...
                {currentStep === 2 && (
                  ...
                  >
                    ...
                    <Button type="button" onClick={handleResheduleAppointment} className="w-full bg-green-500 hover:bg-green-600 hover:cursor-pointer" size="lg" disabled={isSubmitting}>
                      {isSubmitting ? <span>Rescheduling Appointment...</span> : <span>Reschedule Appointment</span>}
                    </Button>
                  </motion.div>
                )}
              </AnimatePresence>
